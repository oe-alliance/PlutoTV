...
#
printf "Creating temporary file PlutoTV-py.pot\n"
find $findoptions .. -name "*.py" -exec xgettext --no-wrap -L Python --from-code=UTF-8 -kpgettext:1c,2 --add-comments="TRANSLATORS:" -d PlutoTV -s -o PlutoTV-py.pot {} \+
$localgsed --in-place PlutoTV-py.pot --expression=s/CHARSET/UTF-8/
printf "Creating temporary file PlutoTV-xml.pot\n"
which python
if [ $? -eq 0 ]; then
	find $findoptions .. -name "setup.xml" -exec python xml2po.py {} \+ > PlutoTV-xml.pot
else
	find $findoptions .. -name "setup.xml" -exec python3 xml2po.py {} \+ > PlutoTV-xml.pot
fi
printf "Merging pot files to create: PlutoTV.pot\n"
